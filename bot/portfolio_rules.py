...
def combine_portfolio_rule(
    ...
    de-risking. Same survivorship convention as the other combiners: a
    missing asset's sleeve sits in cash (exposure shrinks, never reflows).
    """
    ...
    for t in timeline:
        ...
        # crisis de-risking on average pairwise correlation
        exposure = len(present) / n_assets
        if use_crisis:
            corr = avg_pairwise_corr({s: hist[s] for s in present}, corr_window)
            if corr is not None and corr > corr_threshold:
                exposure *= derisk

        gross = sum(weights[s] * asset_dailies[s][t] for s in present)
        out.append(gross * exposure)
        for s in present:
