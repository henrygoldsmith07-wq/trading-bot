...
class PaperExecutionModel:
    ...
    def __post_init__(self) -> None:
        values = (self.half_spread_bps, self.impact_bps, self.max_slippage_bps)
        if any(not math.isfinite(float(v)) or float(v) < 0.0 for v in values):
            raise ValueError("execution model bps parameters must be finite and non-negative")
        if (
            not math.isfinite(float(self.participation_reference))
            or self.participation_reference <= 0.0
        ):
            raise ValueError("participation_reference must be positive and finite")
    ...
    def simulate(
        ...
        """Return an adverse simulated fill and its execution diagnostics."""
        ...
        slippage_bps = min(self.max_slippage_bps, self.half_spread_bps + impact)
        ...
        return PaperFill(fill, slippage_bps, participation)
    ...
    def fill_price(
        ...
        return self.simulate(mid_price, side, order_notional, recent_quote_notional).price
